...
object Util {
  ...
  def formatList(list: Seq[(String, _ <: Any)], entrySep: Char = ',', valueSep: Char = '='): String = {
  ...
  def loadConfigFile(cfg: File): Properties = {
  ...
  def mapTopProp(kvs: Map[_ <: AnyRef, _ <: AnyRef]):Properties = {
    val props = new Properties()
    props.putAll(kvs.asJava)

    props
  }

  def propsPrettyPrint(kvs: Map[_ <: AnyRef, _ <: AnyRef]): String =
    kvs.map {
      case (k, v) => s"$k=$v"
    }.mkString("\n")

  /**
   * convert null and empty string (incl. spaces-only) to None
   */
  def ?(str: String): Option[String] = Option(str).map(_.trim).filter(_.nonEmpty)

  /**
   * Take first non-empty element
   */
  def resolve[A](args: Option[A]*): Option[A] = args.flatten.headOption
